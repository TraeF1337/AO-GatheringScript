...
namespace Ennui.Script.Official
{
    ...
    public class GatheringState : StateScript
    {
        ...
        public override bool OnStart(IScriptEngine se)
        {
            ...
            AddState("rp", new RPFirstState(config, context));
            EnterState("config");
        ...
        public override void OnPaint(IScriptEngine se, GraphicContext g)
        {
            if (config.debugInfo)
            {
                ...
                if (config.debugBg)
                {
                    g.FillRect(theEqualizer, 100, theEqualizer + 265, 325);
                }
                g.SetColor(new Color(1.0f, 1.0f, 1.0f, 1.0f));
                g.DrawString("http://ennui.ninja - Albion Gatherer", theEqualizer, 100);
                ...
                {
                    config.PP1Area.RealArea(Api).Render(Api, Color.Green, Color.Black.MoreTransparent());
                }

                if (config.PP2Area != null && config.PP2Name == Game.ClusterName)
                {
                    config.PP2Area.RealArea(Api).Render(Api, Color.Green, Color.Black.MoreTransparent());
                }

                if (config.PP3Area != null && config.PP3Name == Game.ClusterName)
                {
                    config.PP3Area.RealArea(Api).Render(Api, Color.Green, Color.Black.MoreTransparent());
                }
